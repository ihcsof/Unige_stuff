...
I've built a (really simple and raw) cpp application to let the user train onto DC/SSDM questions.
The app provides you randomly chosen questions from a pool that I'll personally update throughout the course, together with three possible closed answers. You have to choose your preferred answer (only one is right) and the app will let you know whether you were right or not (in that second case, specifying which one was the correct answer). A the very end (either when you finish the questions or when you decide to stop typing "0") you'll be provided with a percentage of correct answers.
...
docker run --pull always -v err:/app/err/ -it lorenzofoschi/questions ./app ssdm.txt
...
2) I chose to learn a bit of Docker because we'll use it into the VCC course (no need to learn something about Docker in order to use this app). However, if you prefer I also put the code and the txt files into this GitHub repo.
